...
THE SCORING LAYER: HOW SCORES ARE PRODUCED

The current prototype implements the scoring layer as a human-in-the-loop evaluation instrument: the supervisor produces the judgment, and the layer structures it into the three-dimensional schema, persists it and displays it. The values are therefore neither the output of an automated scorer (Stage 2 is an integration point, see Pipeline Stage Status below) nor content-independent placeholders: they are structured expert assessments captured by the scoring layer.
...
- Clicking the active button again returns the turn to Pending; evaluations are stored as an override layer and, once saved, survive reloads.
...
- Persistence model: base demo sessions + override layer (server DB when live; localStorage when the backend is unreachable, in which case the DEMO MODE badge is shown — always on static GitHub Pages hosting). Writing is manual via the Save action (see Known Limitations and Scope Boundaries).
...
7. Multi-dimensional column: Pending / Verified (100) / error view with three sliders and feedback field (see The Scoring Layer).
8. Save action: commits the current override layer to the active persistence target (localStorage in demo mode, server DB when live). Until Save is invoked, judgments exist only in memory (see Known Limitations and Scope Boundaries).

KNOWN LIMITATIONS AND SCOPE BOUNDARIES

- Manual persistence: V/X judgments and slider adjustments are held in memory until the Save action is invoked; a session closed before saving loses its evaluations. Autosave or a visible unsaved-changes indicator is the planned refinement; in the prototype, manual saving keeps the persistence event explicit and inspectable.
- Backend trust boundary: the Express endpoint performs no request validation or authentication and writes any JSON payload it receives to sessions-db.json. This is an accepted scope boundary of a local, single-supervisor tool, not a design intended for shared or production deployment.

PIPELINE STAGE STATUS (HONEST)
