...
- The demo-data generation pipeline seeds expert annotations into the typed mocks: the call_03 script defines four realistic failure patterns with differentiated scores — Intent Misclassification (45/20/100), Context Loss / hallucinated course name (100/15/100), Critical Session State Reset (100/0/70), Database Sync Failure (100/10/100) — each with a qualitative classification, demonstrating the scoring schema with non-uniform values.
...
npm install -> npm run setup (synthesizes audio and generates the typed mocks via the Python pipeline; requires Python with the edge-tts package, a local .venv is used automatically if present) -> npm run dev; optionally node server/server.js for live persistence (the DEMO MODE badge then switches off).